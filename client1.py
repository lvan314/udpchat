...
def recMessage():
    while True:
        ...
        if jsonData["type"]=="state":
            if jsonData["state"] and jsonData["state"] == "overtime":
                print(jsonData["content"] + "发送超时")
            elif jsonData["state"] and jsonData["state"]=="back":
                print(jsonData["content"] + "发送失败，对方不在线")
            else:
...
def writeMessage():
    while True:
        ...
        textMessage = TextMessage(sendtime=int(time.time()), type="text", content=message, touserid=2,fromuserid=1)
